...
if __name__ == '__main__':
    ...
    if database == "mysql":
        import records
        db = records.Database(
            'mysql://root:123456@localhost/lianjia?charset=utf8', encoding='utf-8')
    ...
    csv_dir = "{0}/{1}/xiaoqu/{2}/{3}".format(
        DATA_PATH, SPIDER_NAME, city, date)
    ...
    for csv in files:
        with open(csv, 'r') as f:
            for line in f:
                ...
                price = int(price) if price != "0数据" else 0
                sale = int(sale)
                print("{0} {1} {2} {3} {4} {5}".format(
                    date, district, area, xiaoqu, price, sale))
                # 写入mysql数据库
                if database == "mysql":
                ...
                # 写入mongodb数据库
                    ...
                    if not PYTHON_3:
                        worksheet.write_string(
                            row, col, unicode(city_ch, 'utf-8'))
                        worksheet.write_string(row, col + 1, date)
                        worksheet.write_string(
                            row, col + 2, unicode(district, 'utf-8'))
                        worksheet.write_string(
                            row, col + 3, unicode(area, 'utf-8'))
                        worksheet.write_string(
                            row, col + 4, unicode(xiaoqu, 'utf-8'))
    ...
    if database == "excel":
        ...
        json.dump(datas, open('xiaoqu.json', 'w'),
